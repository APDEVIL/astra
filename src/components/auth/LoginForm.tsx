...
export function LoginForm() {
	...
	const [astraPassword, setAstraPassword] = useState("");
	...
	const validate = (): boolean => {
		...
		if (tab === "email") {
			...
			if (!astraId.trim()) next.astraId = "Astra ID is required.";
			if (astraPassword.length < 6)
				next.astraPassword = "Password must be at least 6 characters.";
		}
		setErrors(next);
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			const { error } = await authClient.signIn.email({
				email: tab === "email" ? email : astraId,
				password: tab === "email" ? password : astraPassword,
			});
			if (error) {
